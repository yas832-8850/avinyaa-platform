...
// Uses a single UPDATE...RETURNING so concurrent requests can't collide.
async function getNextJobNumber(orgId: string): Promise<{ jobNumber?: string; error?: string }> {
  const supabase = await createClient();

  // Ensure a sequence exists for this org — create a default one (starts at 1, no prefix) if not
  ...
  // Atomic claim: increment next_number and return the value it had BEFORE incrementing
  ...
  return { jobNumber: formatJobNumber(prefix, claimed_number, padding) };
}
...
export async function createJobMaster(
  ...
  const { jobNumber, error: numberError } = await getNextJobNumber(orgId);
