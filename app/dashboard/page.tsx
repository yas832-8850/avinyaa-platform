    ...
    <div className="mx-auto max-w-4xl p-8">
      <div className="flex items-start justify-between">
        ...
        <div className="flex gap-2">
          ...
          {isSuperAdmin && (
            <>
              <a href="/dashboard/jobs" className="rounded-md border border-gray-300 px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-50">All jobs</a>
              <a href="/dashboard/carriers" className="rounded-md border border-gray-300 px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-50">Manage carriers</a>
              <a href="/dashboard/margin-rules" className="rounded-md border border-gray-300 px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-50">Margin rules</a>
            </>
          )}
        </div>
      </div>
          ...
          <table className="mt-4 w-full text-sm">
            <thead>
              <tr className="border-b text-left text-gray-500">
                ...
                <th className="py-2">{isSuperAdmin ? "Sell rate" : "Rate"}</th>
                  ...
                  <td className="py-2">
                    {job.pods.length > 0 ? (
                      job.pods.map((pod: any) => (
                        <a key={pod.url} href={pod.url} target="_blank" className="mr-2 text-xs text-blue-600 hover:underline">{pod.name}</a>
                      ))
                    ) : (
