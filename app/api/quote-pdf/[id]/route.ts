...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Quote details (right column) — reset y for this column
  let rightY = 760;
  const rightX = 330;
  ...
  page.drawText("Qty", { x: 330, y, size: 9, font: boldFont, color: gray });
  page.drawText("Cost", { x: 370, y, size: 9, font: boldFont, color: gray });
  page.drawText("Margin", { x: 420, y, size: 9, font: boldFont, color: gray });
  page.drawText("Sell/Unit", { x: 470, y, size: 9, font: boldFont, color: gray });
  page.drawText("Total", { x: 520, y, size: 9, font: boldFont, color: gray });
  y -= 8;
  ...
  for (let i = 0; i < lineList.length; i++) {
    ...
    const tier = tierList.find((t) => t.id === line.tier_used_id);

    const descLines = (line.description || "").split("\n");
    for (const descLine of descLines) {
      // wrap long lines roughly by character count since pdf-lib has no auto-wrap
    ...
    y += 12; // move back up to align numeric columns with the first description line
    page.drawText(String(line.order_qty), { x: 330, y, size: 9, font, color: black });
    page.drawText(`$${line.unit_cost.toFixed(2)}`, { x: 370, y, size: 9, font, color: black });
    page.drawText(tier ? `${tier.margin_percent}%` : "—", { x: 420, y, size: 9, font, color: black });
    page.drawText(sellPrice !== null ? `$${sellPrice.toFixed(2)}` : "—", { x: 470, y, size: 9, font, color: black });
    page.drawText(`$${total.toFixed(2)}`, { x: 520, y, size: 9, font, color: black });
    y -= 12;
    y -= 8;
  }
  ...
  if (freightIncluded) {
    ...
    page.drawText(`$${freightCost.toFixed(2)}`, { x: 370, y, size: 9, font, color: black });
    page.drawText(`${freightMargin}%`, { x: 420, y, size: 9, font, color: black });
    page.drawText(`$${freightSell.toFixed(2)}`, { x: 470, y, size: 9, font, color: black });
    page.drawText(`$${freightSell.toFixed(2)}`, { x: 520, y, size: 9, font, color: black });
    y -= 20;
  }
  ...
  const filename = `${quote.quote_number || quote.id.slice(0, 8)}-internal.pdf`;
